...
void RingMultiplier::mult(ZZ* x, ZZ* a, ZZ* b, long np,const ZZ& mod) {
	cuda_host_mult( x,  a,  b,  np, mod);
	
	#ifdef DEBUG_PRINT 
		double ntt_time=0,intt_time=0,crt_time=0,icrt_time=0,mul_time=0;
		...
		cout<<" CPU:  INTT TIME :" << intt_time/1000<<  endl;
		cout<<" CPU:  ICRT TIME :" << icrt_time/1000<<  endl;

		cout<<" CPU: full time = " << (double)chrono::duration_cast<std::chrono::nanoseconds >(end1 - start1).count() / 1000 << " microseconds" << endl;
