...
public:
	...
	uint64_t findMthRootOfUnity(uint64_t M, uint64_t p);
		...
		// cuda function
	void cuda_instanciate(long nprimes, long logN);
	void cuda_host_mult(NTL::ZZ* x, NTL::ZZ* a, NTL::ZZ* b, long np, const NTL::ZZ& mod);
	void cuda_multNTT(NTL::ZZ* x, NTL::ZZ* a, uint64_t* rb, long np, const NTL::ZZ& mod);
	void cuda_multDNTT(NTL::ZZ* x, uint64_t* ra, uint64_t* rb, long np, const NTL::ZZ& mod);
	void cuda_multAndEqual(NTL::ZZ* a, NTL::ZZ* b, long np,const NTL::ZZ& mod);

	void cuda_multNTTAndEqual(NTL::ZZ* a, uint64_t* rb, long np,const NTL::ZZ& mod);
	void cuda_square(NTL::ZZ* x, NTL::ZZ* a, long np,const NTL::ZZ& mod) ;
	void cuda_squareNTT(NTL::ZZ* x, uint64_t* ra, long np,const NTL::ZZ& mod);
	void cuda_squareAndEqual(NTL::ZZ* a, long np,const NTL::ZZ& mod) ;
